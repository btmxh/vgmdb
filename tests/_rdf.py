...
class TestRDF(unittest.TestCase):
    ...
    def load_rdfa_data(self, filename, filterkey=None):
        return self.load_data(
            filename, "html", "rdfa", filterkey=filterkey, media_type="text/html"
        )
    ...
    def run_tests(self, graph, test_count_results, test_first_result):
        ...
        for query, count in list(test_count_results.items()):
            ...
            try:
                ...
                raise
            self.assertEqual(
                len(res), count, "Results for %s: %s vs %s" % (query, len(res), count)
            )
        for query, answer in list(test_first_result.items()):
            ...
            for row in res:
                ...
                self.assertEqual(
                    value,
                    answer,
                    "Result for %s: '%s'(%s) vs '%s'(%s)"
                    % (query, value, type(value), answer, type(answer)),
                )
                break
