...
class TestEventRDF(TestRDF):
    ...
    def run_m3_tests(self, graph):
        ...
        test_first_result = {
            "select ?date where { <@base#release_event> tl:start ?date . }": datetime.date(
                2012, 10, 28
            ),
            "select ?date where { <@base#release_event> tl:end ?date . }": datetime.date(
                2012, 10, 28
            ),
            "select ?date where { <@base#subject> schema:startDate ?date . }": datetime.date(
                2012, 10, 28
            ),
            "select ?date where { <@base#subject> schema:endDate ?date . }": datetime.date(
                2012, 10, 28
            ),
            ...
            'select ?release_date where { ?album dcterms:created ?release_date . ?album schema:name "a Tale"@en . }': datetime.date(
                2012, 10, 28
            ),
            'select ?release_date where { ?album schema:datePublished ?release_date . ?album schema:name "a Tale"@en . }': datetime.date(
                2012, 10, 28
            ),
