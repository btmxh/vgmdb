...
class TestSearchList(unittest.TestCase):
    ...
    def test_search(self):
        ...
        self.assertEqual(
            ...
            search["results"]["albums"][1]["titles"]["en"],
        )
        ...
        self.assertEqual(
            "Vagrant Story", search["results"]["products"][0]["names"]["en"]
        )
        self.assertEqual(
            "ベイグラントストーリー", search["results"]["products"][0]["names"]["ja"]
        )
        self.assertEqual(
            "Vagrant Story", search["results"]["products"][0]["names"]["ja-latn"]
        )
    ...
    def test_search_quotes(self):
        ...
        self.assertEqual(
            "''Snake Eater'' song from METAL GEAR SOLID 3", search["query"]
        )
