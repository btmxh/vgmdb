...
class TestAlbumlistRDF(TestRDF):
    ...
    def run_list_tests(self, graph):
        ...
        test_first_result = {
            "select ?published where { <@basealbum/12991#subject> schema:datePublished ?published . }": datetime.date(
                2004, 0o4, 29
            ),
            "select ?published where { <@basealbum/12991#subject> dcterms:created ?published . }": datetime.date(
                2004, 0o4, 29
            ),
