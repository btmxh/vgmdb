...
class TestArtistsRDF(TestRDF):
    ...
    def run_nobuo_tests(self, graph):
        ...
        test_first_result = {
            ...
            "select ?birthdate where { ?birth rdf:type bio:Birth . ?birth bio:date ?birthdate . }": datetime.date(
                1959, 0o3, 21
            ),
            ...
            "select ?date where { ?album rdf:type schema:MusicAlbum . ?artist foaf:made ?album . ?album schema:datePublished ?date . } order by ?date": datetime.date(
                1986, 0o4, 26
            ),
            "select ?date where { ?album rdf:type mo:Release . ?artist foaf:made ?album . ?album dcterms:created ?date . } order by ?date": datetime.date(
                1986, 0o4, 26
            ),
    ...
    def run_offenbach_tests(self, graph):
        ...
        test_first_result = {
            "select ?date where { ?birth bio:principal <@base#subject> . ?birth a bio:Birth . ?birth bio:date ?date . }": datetime.date(
                1819, 6, 20
            ),
            "select ?date where { ?death bio:principal <@base#subject> . ?death a bio:Death . ?death bio:date ?date . }": datetime.date(
                1880, 10, 0o5
            ),
        }
