...
class TestAlbumsRDF(TestRDF):
    ...
    def run_ff8_tests(self, graph):
        ...
        test_first_result = {
            ...
            "select ?date where { ?album rdf:type schema:MusicAlbum . ?album dcterms:created ?date . }": datetime.date(
                1999, 11, 20
            ),
            ...
            "select ?rating where { <@base#subject> schema:aggregateRating ?agg . ?agg schema:ratingValue ?rating . }": decimal.Decimal(
                "4.44"
            ),
