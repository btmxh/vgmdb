...
class outputter(object):
    ...
    def span_name(self, lang, name, rel="schema:name foaf:name"):
        ...
        if rel:
            return '<span property="%s" lang="%s" xml:lang="%s">%s</span>' % (
                rel,
                lang,
                lang,
                self._escape(name),
            ...
            return '<span lang="%s" xml:lang="%s">%s</span>' % (
                lang,
                lang,
                self._escape(name),
            )
    ...
    def link(self, name, href, typeof=None, hide_empty_link=False):
        ...
        if len(href) > 0 and href[0] == "/":
            href = href[1:]
        if len(href) > 0:
            result = '<a href="%s" about="%s#subject"%s>%s</a>' % (
                href,
                href,
                typeof,
                text,
