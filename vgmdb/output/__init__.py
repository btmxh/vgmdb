...
def unload_module(name):
    global mime_names
    new_mime_names = dict(
        [(k, mime_names[k]) for k in list(mime_names.keys()) if mime_names[k] != name]
    )
...
def load_plugins():
    ...
    for module in list(globals().values()):
        if (
            ...
            and hasattr(module, "mimetypes")
            and hasattr(module, "outputter")
        ):
            try:
                load_module(module)
            except:
