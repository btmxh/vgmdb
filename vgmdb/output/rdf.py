...
def linktype(link, is_type):
    qual_type = is_type + "/"
    if len(link) > len(qual_type) and link[: len(qual_type)] == qual_type:
        return True
    return False
...
def add_discography(g, subject, albums, rel=[FOAF.made, SCHEMA.album], rev=[]):
    for album in albums:
        albumlink = (
            URIRef(link(album["link"]) + "#subject") if "link" in album else BNode()
        )
        perflink = (
            URIRef(link(album["link"]) + "#performance") if "link" in album else BNode()
        )
        complink = (
            URIRef(link(album["link"]) + "#composition") if "link" in album else BNode()
        )
        lyricslink = (
            URIRef(link(album["link"]) + "#lyrics") if "link" in album else BNode()
        )
        vocalslink = (
            URIRef(link(album["link"]) + "#vocals") if "vocals" in album else BNode()
        )
        ...
        if "date" in album:
            g.add(
                (albumlink, DCTERMS.created, Literal(album["date"], datatype=XSD.date))
            )
            g.add(
                (
                    albumlink,
                    SCHEMA.datePublished,
                    Literal(album["date"], datatype=XSD.date),
                )
            )
        if "release_date" in album:
            g.add(
                (
                    albumlink,
                    DCTERMS.created,
                    Literal(album["release_date"], datatype=XSD.date),
                )
            )
            g.add(
                (
        ...
        if "publisher" in album:
            ...
            publisher = (
                URIRef(link(pubdata["link"]) + "#subject")
                if "link" in pubdata
                else BNode()
            )
...
def generate_artist(config, data):
    ...
    if "units" in data:
        for unit in data["units"]:
            unitlink = (
                URIRef(link(unit["link"] + "#subject")) if "link" in unit else BNode()
            )
    ...
    if "members" in data:
        for member in data["members"]:
            memberlink = (
                URIRef(link(member["link"] + "#subject"))
                if "link" in member
                else BNode()
            )
    ...
    if "websites" in data:
        for websitetype, websites in list(data["websites"].items()):
    ...
    if "discography" in data:
        add_discography(
            ...
            subject,
            data["discography"],
            rel=[FOAF.made, SCHEMA.album],
            rev=[DCTERMS.creator, SCHEMA.byArtist],
        )
...
def generate_album(config, data):
    ...
    if "release_date" in data:
        g.add(
            (subject, DCTERMS.created, Literal(data["release_date"], datatype=XSD.date))
        )
        g.add(
            (
                subject,
                SCHEMA.datePublished,
                Literal(data["release_date"], datatype=XSD.date),
            )
        )
    ...
    if "publisher" in data:
        publisher = (
            URIRef(link(data["publisher"]["link"]) + "#subject")
            if "link" in data["publisher"]
            else BNode()
        )
    ...
    if "rating" in data:
        ...
        g.add(
            (rating, SCHEMA.ratingValue, Literal(data["rating"], datatype=XSD.decimal))
        )
        g.add((rating, SCHEMA.bestRating, Literal(5, datatype=XSD.decimal)))
        g.add(
            (rating, SCHEMA.ratingCount, Literal(data["votes"], datatype=XSD.integer))
        )

    def add_people(g, subject, list, rel, rev):
        for persondata in list:
            person = (
                URIRef(link(persondata["link"]) + "#subject")
                if "link" in persondata
                else BNode()
            )
    ...
    if "composers" in data:
        add_people(
            g, subject, data["composers"], rel=[], rev=[DCTERMS.creator, FOAF.made]
        )
        add_people(
            g, composition, data["composers"], rel=[MO.composer], rev=[FOAF.made]
        )
    if "performers" in data:
        ...
        add_people(
            ...
            performance,
            data["performers"],
            rel=[MO.performer, SCHEMA.byArtist],
            rev=[MO.performed],
        )
    ...
    if "products" in data:
        for productdata in data["products"]:
            product = (
                URIRef(link(productdata["link"]) + "#subject")
                if "link" in productdata
                else BNode()
            )
    ...
    for discdata in data["discs"]:
        ...
        g.add(
            (
                record,
                MO.track_count,
                Literal(len(discdata["tracks"]), datatype=XSD.integer),
            )
        )
        g.add(
            (
        ...
        for trackdata in discdata["tracks"]:
            ...
            add_lang_names(
                g, track, trackdata["names"], rel=[SCHEMA.name, DCTERMS.title]
            )
            if "track_length" in trackdata and trackdata["track_length"]:
                interval = "PT" + trackdata["track_length"]
                g.add(
                    (track, SCHEMA.duration, Literal(interval, datatype=XSD.interval))
                )
...
def generate_product(config, data):
    ...
    if "franchises" in data:
        for franchisedata in data["franchises"]:
            franchise = (
                URIRef(link(franchisedata["link"]) + "#subject")
                if "link" in franchisedata
                else BNode()
            )
            g.add((franchise, RDF.type, SCHEMA.CreativeWork))
            add_lang_names(
                g, franchise, franchisedata["names"], rel=[DCTERMS.title, SCHEMA.name]
            )
    if "titles" in data:
        for titledata in data["titles"]:
            title = (
                URIRef(link(titledata["link"]) + "#subject")
                if "link" in titledata
                else BNode()
            )
            ...
            if "date" in titledata:
                g.add(
                    (
                        title,
                        SCHEMA.datePublished,
                        Literal(titledata["date"], datatype=XSD.date),
                    )
                )
                g.add(
                    (
            ...
            add_lang_names(
                g, title, titledata["names"], rel=[DCTERMS.title, SCHEMA.name]
            )
...
def generate_release(config, data):
    ...
    if "products" in data:
        for productdata in data["products"]:
            ...
            g.add((product, RDF.type, SCHEMA.CreativeWork))
            add_lang_names(
                g, product, productdata["names"], rel=[DCTERMS.title, SCHEMA.name]
            )
...
def generate_org(config, data):
    ...
    if "staff" in data:
        for staffdata in data["staff"]:
            staff = (
                URIRef(link(staffdata["link"]) + "#subject")
                if "link" in staffdata
                else BNode()
            )
    ...
    if "websites" in data:
        for websitetype, websites in list(data["websites"].items()):
            for website in websites:
                g.add((subject, FOAF.page, URIRef(website["link"])))
    add_discography(
        g, subject, data["releases"], rel=[MO.published], rev=[MO.publisher]
    )
...
def generate_productlist(config, data):
    ...
    for product_data in data["products"]:
        ...
        add_lang_names(
            g, product, product_data["names"], rel=[SCHEMA.name, DCTERMS.title]
        )
        g.add((product, RDF.type, SCHEMA.CreativeWork))
    return g
...
def generate_eventlist(config, data):
    ...
    if isinstance(data["events"], list):
        ...
        eventlist = []
        for year in list(data["events"].keys()):
    ...
    for event_data in eventlist:
        ...
        g.add(
            (
                event,
                SCHEMA.startDate,
                Literal(event_data["startdate"], datatype=XSD.date),
            )
        )
        ...
        g.add(
            (release_event, TL.at, Literal(event_data["startdate"], datatype=XSD.date))
        )
    return g
...
def generate_recent(config, data):
    ...
    for update in data["updates"]:
        ...
        if "titles" in update:
            add_lang_names(
                g, subject, update["titles"], rel=[SCHEMA.name, DCTERMS.title]
            )
        ...
        if "rating" in update:
            ...
            g.add(
                (
                    rating,
                    SCHEMA.ratingValue,
                    Literal(update["rating"], datatype=XSD.decimal),
                )
            )
            g.add((rating, SCHEMA.bestRating, Literal(5, datatype=XSD.decimal)))
        if "linked" in update:
            ...
            if "titles" in linked_data:
                add_lang_names(
                    g, subject, linked_data["titles"], rel=[SCHEMA.name, DCTERMS.title]
                )
    return g
