...
def do_page(page_type, info, filterkey=None):
    ...
    outputter = vgmdb.output.get_outputter(
        vgmdb.config.for_request(request),
        requested_format,
    ...
    if "meta" in info and "edited_date" in info["meta"]:
        try:
            edited_date = datetime.strptime(
                info["meta"]["edited_date"], "%Y-%m-%dT%H:%M"
            )
            if "ttl" in info["meta"]:
                ttl = info["meta"]["ttl"]
        except:
    ...
    if outputter.content_type[:9] == "text/html":
        sellers = vgmdb.sellers.search_info(
            page_type, info["link"].split("/")[-1], info, start_search=True, wait=False
        )
    ...
    if edited_date:
        ...
        out_time = email.utils.formatdate(
            timeval=unix_time, localtime=True, usegmt=True
        )
...
@route(
    "/recent/<type:re:(albums|media|tracklists|scans|artists|products|labels|links|ratings)>"
)
@route("/recent")
@catch_exceptions
@instrumented
...
def about():
    outputter = vgmdb.output.get_outputter(
        vgmdb.config.for_request(request), "html", None
    )
    response.content_type = outputter.content_type
...
def sellers(type, id):
    ...
    sellers = vgmdb.sellers.search(
        type, id, start_search=True, wait=True, allow_partial=allow_partial
    )
    searching = any(["searching" in item for item in sellers])
    requested_format = request.query.format or ""
    outputter = vgmdb.output.get_outputter(
