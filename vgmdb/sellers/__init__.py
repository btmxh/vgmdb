        ...
        with Timer(tag=module.__name__, verbose=False):
            ...
            if search and not prev:
                try:
                    ...
                    logger.error(
                        "Exception while searching %s for %s/%s: %s"
                        % (module, type, id, e)
                    )
                if ret:
                    cache.set(
                        "vgmdb/%s/%s/sellers/%s" % (type, id, module.__name__), ret
                    )
                    results.append(ret)
                else:
...
def get_results(type, id, info):
    results = []
    for module in search_modules:
        module_results = cache.get(
            "vgmdb/%s/%s/sellers/%s" % (type, id, module.__name__)
        )
