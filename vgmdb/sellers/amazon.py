...
if hasattr(config, "AMAZON_ASSOCIATE_TAG") and config.AMAZON_ASSOCIATE_TAG:
    SEARCH_PAGE = (
        "http://www.amazon.com/s/?_encoding=UTF8&url=search-alias%%3Dpopular&SubscriptionId=%s&AssociateTag=%s&linkCode=ur2&tag=%s"
        % (
...
if (
    hasattr(config, "AMAZON_ACCESS_KEY_ID")
    and config.AMAZON_ACCESS_KEY_ID
    and hasattr(config, "AMAZON_SECRET_ACCESS_KEY")
    and config.AMAZON_SECRET_ACCESS_KEY
):
...
def run_search(keywords, artist=None):
    search_items_resource = [
        paapi5.search_items_resource.SearchItemsResource.ITEMINFO_TITLE
    ]
...
def empty_album(info):
    search_url = SEARCH_PAGE + "&field-keywords=%s" % (
        urllib.parse.quote(squash_str(info["name"]))
    )
    result = {"name": "Amazon", "icon": "static/amazon.png", "search": search_url}
    return result
...
def search_artist_album_name(info):
    ...
    title = info["name"]
    results = run_search(
        keywords=squash_str(artist) + " " + squash_str(title), artist=squash_str(artist)
    )
    if results is None:
        return None
    found = find_best_match(
        ...
        results,
        threshold=0.5,
        key=lambda x: squash_str(x.item_info.title.display_value),
    )
    return found
...
def empty_artist(info):
    search_url = SEARCH_PAGE + "&field-artist=%s" % (
        urllib.parse.quote(squash_str(info["name"])),
    )
    result = {"name": "Amazon", "icon": "static/amazon.png", "search": search_url}
    return result
