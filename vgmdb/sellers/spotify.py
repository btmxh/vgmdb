...
def authenticate():
    ...
    access_expires = datetime.datetime.utcnow() + datetime.timedelta(
...
def empty_album(info):
    search_url = "https://play.spotify.com/search/%s%%3Aalbums" % (
        urllib.parse.quote(squash_str(info["name"])),
    )
    result = {"name": "Spotify", "icon": "static/spotify.png", "search": search_url}
    return result
...
def search_artist_album_name(info):
    ...
    found = find_best_match(
        ...
        data["albums"]["items"],
        threshold=0.5,
        key=lambda x: squash_str(x["name"]),
    )
    return found
...
def empty_artist(info):
    search_url = "https://play.spotify.com/search/%s%%3Aartists" % (
        urllib.parse.quote(squash_str(info["name"])),
    )
    result = {"name": "Spotify", "icon": "static/spotify.png", "search": search_url}
    return result
