...
def empty_album(info):
    search_url = SEARCH_API + "entity=album&term=%s" % (
        urllib.parse.quote(squash_str(info["name"])),
    )
    result = {"name": "iTunes", "icon": "static/itunes.png"}
    return result
...
def search_album_name(info):
    ...
    found = find_best_match(
        ...
        data["results"],
        threshold=0.5,
        key=lambda x: squash_str(x["collectionName"]),
    )
    return found


def empty_artist(info):
    search_url = SEARCH_API + "entity=musicArtist&term=%s" % (
        urllib.parse.quote(squash_str(info["name"])),
    )
    result = {"name": "iTunes", "icon": "static/itunes.png"}
    return result
...
def search_artist_name(name):
    url = SEARCH_API + "entity=musicArtist&term=%s" % (
        urllib.parse.quote(squash_str(name)),
    )
    webdata = urllib.request.urlopen(url).read()
    data = json.loads(webdata)
    found = find_best_match(
        ...
        data["results"],
        threshold=0.7,
        key=lambda x: squash_str(x["artistName"]),
    )
    return found
