...
def search(query):
    ...
    if hasattr(config, "DISCOGS_KEY") and hasattr(config, "DISCOGS_SECRET"):
        request.add_header(
            "Authorization",
            "Discogs key=%s, secret=%s" % (config.DISCOGS_KEY, config.DISCOGS_SECRET),
        )
...
def empty_album(info):
    search_url = "http://www.discogs.com/search?q=%s&type=master" % (
        urllib.parse.quote(squash_str(info["name"])),
    )
    result = {"name": "discogs", "icon": "static/discogs.ico", "search": search_url}
    return result
...
def search_album_catalog(catalog):
    webdata = search(
        "type=master&catno=%s" % (urllib.parse.quote(squash_str(catalog)),)
    )
    ...
    found = find_best_match(
        ...
        data["results"],
        threshold=0.9,
        key=lambda x: squash_str(x["catno"]),
    )
    return found
...
def search_artist_album_name(info):
    ...
    title = info["name"]
    webdata = search(
        "type=master&artist=%s&release_title=%s"
        % (
            urllib.parse.quote(squash_str(artist)),
            urllib.parse.quote(squash_str(title)),
        )
    )
    data = json.loads(webdata)
    found = find_best_match(
...
def search_album_name(info):
    title = info["name"]
    webdata = search(
        "type=master&release_title=%s" % (urllib.parse.quote(squash_str(title)),)
    )
    data = json.loads(webdata)
    found = find_best_match(
        squash_str(title),
...
def empty_artist(info):
    search_url = "http://www.discogs.com/search?q=%s&type=artist" % (
        urllib.parse.quote(squash_str(info["name"])),
    )
    result = {"name": "discogs", "icon": "static/discogs.ico", "search": search_url}
    return result
