...
def parse_page(html_source):
    ...
    productlist_info["meta"]["time"] = str(soup_metadata.div.b.string)
    soup_letters = soup_sections[1]
    productlist_info["letters"] = []
    for soup_letter in soup_letters.find_all("td"):
        if soup_letter.a:
            letter = str(soup_letter.a.string)
        if soup_letter.strong:
            letter = str(soup_letter.strong.string)
...
def _parse_product(soup_row):
    ...
    product_type = str(soup_span.string)

    # parse title
    soup_link = soup_name.a
    product_link = soup_link["href"]
    product_link = utils.trim_absolute(product_link)
    product_name = str(soup_link.string)

    return {"link": product_link, "type": product_type, "names": {"en": product_name}}
