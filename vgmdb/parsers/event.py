...
def parse_page(html_source):
    ...
    event_info["name"] = (
        soup_titlespans[1].find("span", class_="albumtitle").string.strip()
    )
    ...
    event_info["releases"] = _parse_event_releases(
        soup_sections[2].find("table", recursive=False)
    )
...
def _parse_event_releases(soup_table):
    ...
    for soup_row in soup_rows[1:]:
        ...
        release["release_date"] = utils.parse_date_time(
            soup_cells[5].span.string.strip()
        )
        ...
        if soup_cells[4].a:
            ...
            release["publisher"] = {"names": {"en": soup_cells[4].string.strip()}}
