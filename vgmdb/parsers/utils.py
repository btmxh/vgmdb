...
def url_list_page(type, id):
    ...
    if len(id) > 1:
        page = int(id[1:])
    return "https://vgmdb.net/db/%s.php?ltr=%s&field=title&perpage=100&page=%s" % (
        type,
        id,
        page,
    )
...
def is_english(text):
    ...
    trimmed = list(filter(is_letter, decomposed))
    trimmed = [extra_normalize(char) for char in trimmed]
    count = len(trimmed)
    count_english = len(list(filter(is_english_char, trimmed)))
    if count == 0:
...
def parse_date_time(time):
    ...
    day_year_found = re.match(
        "([0-9][0-9]?),? ?([12][0-9][0-9][0-9])?", time[space + 1 :]
    )
...
def parse_discography(soup_disco_table, label_type="roles"):
    ...
    for soup_tbody in soup_disco_table.find_all("tbody", recursive=False):
        ...
        year = str(soup_rows[0].find("h3").string)
        for soup_album_tr in soup_rows[1:]:
            soup_cells = soup_album_tr.find_all("td")
            month_day = str(soup_cells[0].string)
            ...
            catalog = str(soup_album_info[0].string)
            ...
            for soup_tag in soup_cells[1].find_all("img", recursive=False):
                if (
                    soup_tag.has_attr("alt")
                    and soup_tag["alt"] == "This album is a reprint"
                ):
                    reprint = True
