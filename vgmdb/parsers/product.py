...
def parse_page(html_source):
    ...
    if soup_real_name:
        ...
        if len(soup_real_name.contents) == 1:
            product_info["name_real"] = str(soup_real_name.string)
        elif len(soup_real_name.contents) > 1:
            product_info["name_real"] = str(soup_real_name.contents[0].string)
    ...
    for soup_section_head in soup_section_heads:
        ...
        if section_name == "Belongs to":
            product_info["superproduct"] = _parse_franchise_superproduct(
                soup_section.div.div
            )
        if section_name == "Subproducts":
            product_info["subproducts"] = _parse_franchise_subproducts(
                soup_section.div.table
            )
        ...
        if section_name == "Albums | Credits" or section_name == "Albums":
            product_info["albums"] = utils.parse_discography(
                soup_section.div.table, "classifications"
            )
    ...
    for soup_right_section in soup_right_divs[:-1]:
        if "rtop" in soup_right_section.find("b", recursive=False)["class"]:
            section_head = str(soup_right_section.div.h3.string)
        if "rbot" in soup_right_section.find("b", recursive=False)["class"]:
            ...
            if section_head == "Websites":
                product_info["websites"] = {}
                for soup_website_section in soup_section_body.find_all(
                    "div", recursive=False
                ):
                    website_type = str(soup_website_section.b.string)
                    websites = []
                    for soup_site in soup_website_section.find_all(
                        "a", recursive=False
                    ):
                        link = utils.strip_redirect(soup_site["href"])
                        name = str(soup_site.string)
        ...
        if soup_child.name == "dd":
            if soup_child.div:
                ...
                for soup_child_div in soup_child.find_all("div", recursive=False):
                    for soup_child_link in soup_child_div.find_all(
                        "a", recursive=False
                    ):
                        item = {}
...
def _parse_product_releases(soup_table):
    ...
    for soup_row in soup_rows[1:]:
        ...
        release["region"] = str(soup_cells[2].span.string)
        release["platform"] = str(soup_cells[3].span.string)
