...
def _parse_album_info(soup_info):
    ...
    album_info = {}
    if "class" in soup_info.attrs and soup_info["class"][0] == "bootleg":
        ...
        if name == "Catalog Number":
            ...
            if isinstance(soup_children[0], bs4.Tag):
                if soup_children[0].name == "a":  # link to calendar
                    ...
                    if date.isdigit():
                        album_info["release_date"] = "%s-%s-%s" % (
                            date[0:4],
                            date[4:6],
                            date[6:8],
                        )
                    else:  # link to calendar, but date is ambiguous (Jul 1997)
                        album_info["release_date"] = utils.parse_date_time(
                            soup_value.a.string
                        )
            ...
            for soup_event in soup_children[1:]:
                if not isinstance(soup_event, bs4.Tag):
                    continue
                ...
                event["shortname"] = str(soup_event.string)
                event["link"] = link
            ...
            if len(soup_links) == 0:
                addOrganization(
                    {"names": {"en": soup_value.string.strip()}}, "publisher"
                )
            # Organizations row has some expando-javascript
            for soup_child in soup_value.children:
                if (
                    isinstance(soup_child, bs4.Tag)
                    and soup_child.name == "span"
                    and soup_child.get("id") == "publisher_less"
                ):
            ...
            for soup_child in soup_value.children:
                if isinstance(soup_child, bs4.Tag) and soup_child.name == "a":
                    ...
                    text = str(soup_child.string).strip()
                    splits = text.split("(", 1)
...
def _parse_right_column(soup_right_column):
    ...
    while soup_div:
        ...
        if soup_div.div.h3:
            section_title = str(soup_div.div.h3.string)
            ...
            if section_title == "Covers":
                ...
                soup_section = (
                    soup_div.find_next_sibling()
                )  # allow the next_sibling loop to work
                if not soup_section:
...
def _parse_section_related_albums(soup_div):
    ...
    for soup_album in soup_div.find_all("div", recursive=False):
        ...
        if soup_album.ul:  # if there are thumbnails
            ...
            catalog = str(soup_album.find("span", recursive=False).string)
