...
def parse_page(html_source):
    ...
    if (
        len(spans) > 2 and spans[2]["class"] and "time" in spans[2]["class"]
    ):  # artist type (unit)
        soup_type = spans[2]
    ...
    (soup_profile_left, soup_profile_right) = soup_profile.find_all(
        "div", recursive=False, limit=2
    )
    ...
    if soup_disco_div:
        artist_info["discography"] = utils.parse_discography(
            soup_disco_div.find("table"), "roles"
        )
    else:
        artist_info["discography"] = []
    soup_featured_div = disco_container.find(id="featuredtable")
    if soup_featured_div:
        artist_info["featured_on"] = utils.parse_discography(
            soup_featured_div.find("table"), "roles"
        )
    else:
    ...
    if (
        soup_divs[0].div
        and soup_divs[0].div.h3
        and soup_divs[0].div.h3.string == "Websites"
    ):
    ...
    for soup_item in soup_profile_left.find_all("div", recursive=False)[1:]:
        ...
        while list_item_pre:
            ...
            if isinstance(soup_item_data, bs4.Tag):
                ...
                # rating stars
                if (
                    soup_item_data.name == "div"
                    and soup_item_data.has_attr("class")
                    and "star" in soup_item_data["class"]
                ):
                ...
                if (
                    soup_item_data.name == "span"
                    and soup_item_data.has_attr("class")
                    and "time" in soup_item_data["class"]
                ):
        ...
        if (
            len(item_list) == 1
            and isinstance(item_list[0], str)
            and item_name not in people_lists
        ):
