...
def _parse_artist_info(soup_cell, append_target):
    if soup_cell.a:
        ...
        name = str(soup_artist.string)
        artist_info = {"link": utils.trim_absolute(link), "names": {"en": name}}
        if soup_cell.span:
            name_real = str(soup_cell.span.string)
