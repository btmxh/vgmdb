    ...
    for list, section in (
        ...
        for letter in "#ABCDEFGHIJKLMNOPQRSTUVWXYZ":
            ...
            while id:
                ...
                id = (
                    data["pagination"].get("link_next", "/").split("/")[-1]
                )  # next page
...
def search_locally(query):
    ...
    for section, data in SEARCH_INDEX.items():
        ...
        if not all(
            piece[:16].encode("utf-8") in BLOOM_INDEX[section] for piece in pieces
        ):
        ...
        for item in data:
            ...
            # others
                ...
                sections[section].append(item)
    logger.debug(
        "Searching for %s locally took %s"
        % (
            pieces,
            time.time() - start,
        )
