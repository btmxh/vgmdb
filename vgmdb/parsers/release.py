...
def parse_page(html_source):
    ...
    if soup_real_name:
        ...
        if len(soup_real_name.contents) == 1:
            release_info["name_real"] = str(soup_real_name.string)
        elif len(soup_real_name.contents) > 1:
            release_info["name_real"] = str(soup_real_name.contents[0].string)
    ...
    for soup_section_head in soup_section_heads:
        section_name = str(soup_section_head.string)
        ...
        if section_name == "Albums | Credits":
            ...
            if maybe_soup_table.name == "table":
                release_info["release_albums"] = utils.parse_discography(
                    maybe_soup_table, "classifications"
                )
            else:
                release_info["release_albums"] = []
            maybe_soup_table = utils.next_sibling_tag(soup_discos[1])
            if maybe_soup_table.name == "table":
                release_info["product_albums"] = utils.parse_discography(
                    maybe_soup_table, "classifications"
                )
            else:
                release_info["product_albums"] = []
