...
def parse_page(html_source):
    ...
    recent_info["updates"] = _parse_table(
        cur_section, color_codes, soup_innermain.find("table")
    )
...
def _parse_table_media(color_codes, soup_cells):
    ...
    info["catalog"] = str(soup_cells[0].string)
...
def _parse_table_links(color_codes, soup_cells):
    ...
    info.update(_parse_contributor_cell(soup_cells[2]))
    soup_link = soup_cells[0].a
    if "link_type" in info and info["link_type"] in ["Album Link", "Purchase Link"]:
        info.update(
            {
                "link": utils.trim_absolute(soup_link["href"]),
                "catalog": str(soup_link.string),
    ...
    if "link_type" in info and info["link_type"] in ["Artist Link"]:
        soup_link = soup_cells[0].a
        info.update(
            {
                "link": utils.trim_absolute(soup_link["href"]),
    ...
    if "link_type" in info and info["link_type"] in [
        ...
        info.update({"link": item_link, "names": {"en": str(soup_link.string)}})
    return info
...
def _parse_title_cell(type, color_codes, soup_cell):
    ...
    if soup_link:
        ...
        if (
            soup_link.span
            and soup_link.span.span
            and soup_link.span.span.has_attr("lang")
        ):
            soup_title = soup_link.span
...
def _parse_contributor_cell(soup_cell):
    ...
    date_str = str(soup_date) + str(soup_date.next_element.string)
