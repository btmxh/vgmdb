...
class RedisCache(object):
    ...
    def __setitem__(self, key, value):
        try:
            if (
                key.startswith("vgmdb/albumlist")
                or key.startswith("vgmdb/artistlist")
                or key.startswith("vgmdb/orglist")
                or key.startswith("vgmdb/eventlist")
                or key.startswith("vgmdb/productlist")
            ):
...
if not cache and memcache:
    try:
        logger.info(
            "Connecting Memcache client to %s with args %s"
            % (config.MEMCACHE_SERVERS, config.MEMCACHE_ARGS)
        )
